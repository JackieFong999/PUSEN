...
<style>
  ...
  #advisorGrid .ag-root-wrapper { border: none; }
  ...
  #advisorGrid .ag-header-cell-text { font-size: .7rem; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; }
  #advisorGrid .ag-cell { display: flex; align-items: center; }
  #advisorGrid .ag-paging-panel { font-size: .8rem; color: var(--text-muted); border-top: 1px solid var(--card-border); }

  /* ---------- loading overlay ---------- */
  #advisorGrid .ag-overlay { background: color-mix(in srgb, var(--card-bg) 45%, transparent); }
...
<div class="stat-card mb-2">
  <form id="advisorSearchForm" class="row g-3 align-items-end">
    <div class="col-md-3">
      <label class="form-label" for="fAdvisor">Advisor ID</label>
      <input type="text" class="form-control" id="fAdvisor" name="advisor_id"
             list="advisorIdList" placeholder="e.g. jordan01" autocomplete="off">
      <datalist id="advisorIdList">
        @foreach ($options['advisorIds'] as $a)
    ...
    <div class="col-md-3">
      ...
      <input type="text" class="form-control" id="fStudentId" name="student_id"
             list="studentIdList" placeholder="e.g. 25888888D" autocomplete="off">
      <datalist id="studentIdList">
        @foreach ($options['studentIds'] as $s)
          <option value="{{ $s }}"></option>
        @endforeach
      </datalist>
    </div>
    <div class="col-md-3">
      <label class="form-label" for="fAdvisorType">Advisor Type</label>
      <select class="form-select" id="fAdvisorType" name="advisor_type">
        <option value="">-- All --</option>
        @foreach ($options['advisorTypes'] as $t)
...
<script nonce="{{ $cspNonce }}">
  ...
  const gridOptions = {
    columnDefs: [
      { field: 'advisorId', headerName: 'Advisor ID', width: 150 },
      { field: 'studentId', headerName: 'Student ID', width: 130 },
      { field: 'advisorType', headerName: 'Advisor Type', flex: 1 },
      { field: 'startDate', headerName: 'Start Date', width: 120, cellRenderer: p => p.value ?? '—' },
      { field: 'endDate', headerName: 'End Date', width: 120, cellRenderer: p => p.value ?? '—' },
    ],
    ...
    getRowId: p => p.data.advisorId + '-' + p.data.studentId + '-' + p.data.advisorType + '-' + p.data.startDate,
    onGridReady: p => { gridApi = p.api; },
    suppressCellFocus: true,
    loadingOverlayComponent: AdvisorLoadingOverlay,
  };

  const gridEl = document.getElementById('advisorGrid');
  ...
  /* ---------- search ---------- */
  document.getElementById('advisorSearchForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const res = await fetch('/admin/advisor-list/search?' + params.toString(), { headers: { 'Accept': 'application/json' } });
  ...
  document.getElementById('resetBtn').addEventListener('click', () => {
    document.getElementById('advisorSearchForm').reset();
