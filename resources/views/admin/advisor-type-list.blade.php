...
<div class="page-header d-flex flex-wrap align-items-end justify-content-between gap-3 mb-4">
  <div>
    <h1 class="mb-1">Advisor Type</h1>
    <p>Advisor types used in the system — retrieved from <code>tblAdvisor_Type</code>.</p>
  </div>
</div>
...
<div class="stat-card p-0 overflow-hidden">
  <div class="table-responsive">
    <table class="table table-hover align-middle mb-0" style="min-width: 480px;">
      <thead>
        <tr>
          <th style="width:220px;">Advisor Type</th>
          <th>Advisor Type Description</th>
        </tr>
      </thead>
      <tbody>
        @forelse ($advisorTypes as $at)
          <tr>
            <td><span class="tag">{{ $at->Advisor_Type }}</span></td>
            <td>{{ $at->Advisor_Type_Desc }}</td>
          </tr>
        @empty
          <tr>
            <td colspan="2" class="text-center text-muted py-4">No advisor types found.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
...
<div class="text-center mt-4">
  <span style="font-size:.78rem; color:var(--text-faint);">
    <i class="bi bi-database me-1"></i>{{ count($advisorTypes) }} record(s) from PUSENDB.tblAdvisor_Type
  </span>
</div>
