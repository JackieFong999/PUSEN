...
<div class="page-header d-flex flex-wrap align-items-end justify-content-between gap-3 mb-4">
  <div>
    <h1 class="mb-1">Subject Type</h1>
    <p>Subject types used in the system — retrieved from <code>tblSubject_Type</code>.</p>
  </div>
</div>
...
<div class="stat-card p-0 overflow-hidden">
  <div class="table-responsive">
    <table class="table table-hover align-middle mb-0" style="min-width: 480px;">
      ...
      <tbody>
        @forelse ($subjectTypes as $st)
          <tr>
            <td><span class="tag">{{ $st->Subject_Type }}</span></td>
          </tr>
        @empty
          <tr>
            <td class="text-center text-muted py-4">No subject types found.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
...
<div class="text-center mt-4">
  <span style="font-size:.78rem; color:var(--text-faint);">
    <i class="bi bi-database me-1"></i>{{ count($subjectTypes) }} record(s) from PUSENDB.tblSubject_Type
  </span>
</div>
