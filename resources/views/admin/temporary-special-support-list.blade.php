...
<div class="stat-card p-0 overflow-hidden">
  <div class="table-responsive">
    <table class="table table-hover align-middle mb-0" style="min-width: 480px;">
      <thead>
        <tr>
          <th>Temporary Special Support</th>
        </tr>
      </thead>
      <tbody>
        @forelse ($supports as $s)
          <tr>
            <td><span class="tag">{{ $s->Temporary_Special_Support }}</span></td>
          </tr>
        @empty
          <tr>
            <td class="text-center text-muted py-4">No temporary special supports found.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
...
<div class="text-center mt-4">
  <span style="font-size:.78rem; color:var(--text-faint);">
    <i class="bi bi-database me-1"></i>{{ count($supports) }} record(s) from PUSENDB.tblTemporary_Special_Support
  </span>
</div>
