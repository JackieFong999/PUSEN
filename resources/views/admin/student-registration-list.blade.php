...
<div class="page-header d-flex flex-wrap align-items-end justify-content-between gap-3 mb-4">
  <div>
    <h1 class="mb-1">Student Registration</h1>
    <p>Student–subject registrations — retrieved from <code>tblStudent_Reg</code>.</p>
  </div>
</div>
      ...
      <tbody>
        @forelse ($registrations as $r)
          <tr>
            ...
            <td>{{ $r->Student_Id }}</td>
            <td><span class="tag">{{ $r->Subject_Code }}</span></td>
          </tr>
        @empty
          <tr>
            <td colspan="3" class="text-center text-muted py-4">No registrations found.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
...
<div class="text-center mt-4">
  <span style="font-size:.78rem; color:var(--text-faint);">
    <i class="bi bi-database me-1"></i>{{ count($registrations) }} record(s) from PUSENDB.tblStudent_Reg
  </span>
</div>
