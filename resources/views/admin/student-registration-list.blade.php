...
<style nonce="{{ $cspNonce }}">
  ...
  #studentRegGrid .ag-root-wrapper { border: none; }
  ...
  #studentRegGrid .ag-header-cell-text { font-size: .7rem; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; }
  #studentRegGrid .ag-cell { display: flex; align-items: center; }
  #studentRegGrid .ag-paging-panel { font-size: .8rem; color: var(--text-muted); border-top: 1px solid var(--card-border); }

  /* ---------- loading overlay ---------- */
  #studentRegGrid .ag-overlay { background: color-mix(in srgb, var(--card-bg) 45%, transparent); }
...
<div class="stat-card mb-2">
  <form id="studentRegSearchForm" class="row g-3 align-items-end">
    <div class="col-md-2">
      <label class="form-label" for="fStudentId">Student ID</label>
      <input type="text" class="form-control" id="fStudentId" name="student_id"
             list="studentIdList" placeholder="e.g. 25111111G" autocomplete="off">
...
{{-- ============ AG GRID ============ --}}
<div id="studentRegGrid" class="ag-theme-alpine-dark"></div>
...
<script nonce="{{ $cspNonce }}">
  ...
  class StudentRegLoadingOverlay {
    init() {
  ...
  const gridOptions = {
    columnDefs: [
      ...
      { field: 'createdAt', headerName: 'Created At', width: 170, cellRenderer: p => p.value ?? '—' },
    ],
    ...
    getRowId: p => p.data.studentId + '-' + p.data.subjectCode,
    onGridReady: p => { gridApi = p.api; },
    suppressCellFocus: true,
    loadingOverlayComponent: StudentRegLoadingOverlay,
  };

  const gridEl = document.getElementById('studentRegGrid');
  ...
  /* ---------- search ---------- */
  document.getElementById('studentRegSearchForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const res = await fetch('/admin/student-registration-list/search?' + params.toString(), { headers: { 'Accept': 'application/json' } });
  ...
  document.getElementById('resetBtn').addEventListener('click', () => {
    document.getElementById('studentRegSearchForm').reset();
